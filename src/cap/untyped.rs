...
use selfe_wrap::{
    CNodeCptr, CNodeKernel, FullyQualifiedCptr, FullyQualifiedCptrSpan, Guard, SelfeKernel,
};
...
impl<BitSize: Unsigned> LocalCap<Untyped<BitSize, memory_kind::General>> {
        ...
        BitSize: IsGreaterOrEqual<Sum<ChildRadix, CNodeSlotBits>>,
    {
        ...
        let scratch = scratch_slot.elim().cptr;
        let destination = dest_slot.elim().cptr;
        ...
        unsafe {
            ...
            seL4_Untyped_Retype(
                ...
                scratch.cnode.into(),                    // root
                0,                                       // index
                0,                                       // depth
                scratch.index.into(),                    // offset
                1,                                       // num_objects
            )
            .as_result()
            .map_err(|e| SeL4Error::new(APIMethod::UntypedRetype, e))?;
        ...
        SelfeKernel::cnode_mutate(
            scratch,
            destination,
            Guard::empty(seL4_WordBits as u8 - ChildRadix::U8).into(),
        )
        .map(|dest| {
            (
                Cap {
                ...
                CNodeSlots::internal_new(dest.index.into(), 1),
            )
        })
