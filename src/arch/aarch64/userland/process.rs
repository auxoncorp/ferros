...
pub(crate) unsafe fn setup_initial_stack_and_regs(
    ...
    if param_fits_in_regs {
        ...
        let sp = (stack_top as *mut u8).sub(padded_param_size);
        ptr::copy_nonoverlapping(param as *const u8, sp, padded_param_size);
    }

    (regs, padded_param_size)
}
